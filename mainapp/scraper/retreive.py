...
from cpapi.settings import GITHUB_API
...
def Leetcode_retreive_fn(username):
	...
	ret['languages'] = []
	ret['skills_advanced']  = []
	ret['skills_intermediate']  = []
	ret['skills_fundamental']  = []
	ret['contests'] = []
	ret['badges'] = []
...
def Codechef_retreive_fn(username):
	...
	if res.status_code==200:
		...
		try:
			...
			try:
				problems_solved = 0
				different_problem_category = soup.find_all("section",class_="rating-data-section problems-solved")[0].find_all("h3")
				for i in different_problem_category:
    					problems_solved= problems_solved + int(i.text.strip().split('(')[1].split(')')[0])
